 ...
 * Bounded to 8192 players × 32 recent sites; ordinary movement is never evidence of cheating.
 */
class StationaryHuntGuard(private val settings: () -> HuntGuardSettings) {
    ...
    private data class Site(
        val target: Position, var hunter: Position, var startedAt: Long, var lastAt: Long,
    ...
    fun recordKill(playerId: UUID, hunter: Position, target: Position, now: Long): Boolean {
        val config = settings()
        ...
        val site = player.sites.firstOrNull { it.target.near(target, config.siteRadius) }
            ?: Site(target, hunter, now, now).also {
        ...
        if (site.blockedUntil != 0L || !site.hunter.near(hunter, config.hunterRadius)) {
            site.hunter = hunter
            site.startedAt = now
            site.kills = 0
            site.blockedUntil = 0
...
data class HuntGuardSettings(
    ...
    init {
        ...
        require(hunterRadius.isFinite() && hunterRadius in 2.0..32.0)
